...
class DQN(agent.AgentBase):
    ...
    def _build_network(self):
        ...
        with tf.name_scope('action_mask'):
            ...
            action_mask = tf.eye(tf.size(action))[action]
    ...
    def _perceive(self, state, action, state_, reward, done):
        ...
        if self._replay_buffer.is_full():
            ...
            state_batch, action_batch, next_state_batch, reward_batch, done_batch = \
                self._replay_buffer.get_batch(n_batch=64, n_lost=32)
            #
            reward_batch = [-100 if done else reward for reward, done in zip(reward_batch, done_batch)]
            #
            q_current_batch = self._sess.run(self._tensor['q_current'], feed_dict={self._tensor['state']: state_batch,
                                                                                   self._tensor[
                                                                                       'action']: action_batch})
            #
